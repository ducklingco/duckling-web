      ...
      <div v-if="!isFullscreenLocal" class="absolute right-0 top-0 flex items-center gap-2 pr-3 pt-2 sm:pr-8 sm:pt-2">
        ...
        >
          ...
          <svg v-else class="animate-spin" width="29" height="29" viewBox="0 0 29 29" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="14.5" cy="14.5" r="12" :stroke="navFillColor" stroke-opacity="0.3" stroke-width="2.5"/>
            <path d="M14.5 2.5C21 2.5 26.5 8 26.5 14.5" :stroke="navFillColor" stroke-opacity="0.8" stroke-width="2.5" stroke-linecap="round"/>
          </svg>
        </button>
      </div>
...
const handleWheel = (e: WheelEvent) => {
  ...
  e.preventDefault();
  const now = Date.now();
  if (now - lastWheelTime.value < 1000) return;
  lastWheelTime.value = now;
  isSwiping.value = true;
  setTimeout(() => { isSwiping.value = false; }, 1000);
